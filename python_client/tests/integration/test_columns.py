...
def test_columns_search():
    # routes: 818
    # AccumuloControllerColumnNames
    # DataScanSpec
    # POST /columns/search controllers.accumulo.AccumuloControllerColumnNames.searchColumnNames(request: Request)
    body = {
    ...
    req_path = f'{BASE_API_PATH}/columns/search'
    res = reqpost(req_path, headers=AUTH_HEADERS, json=body)
    assert res.status_code == 200
    # not sure what to check, but found ok data to send
