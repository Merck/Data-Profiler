...
def test_tables_search():
    # routes: 833
    # AccumuloControllerTables
    # POST /tables/search controllers.accumulo.AccumuloControllerTables.searchTableNames(request: Request)
    body = {
    ...
    req_path = f'{BASE_API_PATH}/tables/search'
    res = reqpost(req_path, headers=AUTH_HEADERS, json=body)
    assert res.status_code == 200
    assert res.json() != {}  # nothing to search?
