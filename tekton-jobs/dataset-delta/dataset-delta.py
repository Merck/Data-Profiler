...
logger = getLogger('dataset-delta')
...
def execute(conf: Config, job: Job) -> bool:
    """
    run dataset delta, set job status to complete
    :param conf:
    :param job:
    :return:
    """
    if job.type != JobType.DATASETDELTA:
        msg = 'Tried to execute job as a dataset delta job, but it was not a dataset delta job.'
        logger.error(msg)
    ...
    with NamedTemporaryFile(delete=False) as fd:
        logger.info('Writing out dataset delta spec...')
        logger.debug(job.details.to_json())
        fd.write(job.details.to_json().encode())
        fd.flush()
        logger.info('Calling dataset delta cli...')
        args = ["--fname", fd.name]
        class_name = "com.dataprofiler.datasetdelta.cli.DatasetDeltaCli"
        heap_size = '1024m'
        jvm_args = ['-Xms' + heap_size, '-Xmx' + heap_size]
        try:
            run_java_command(conf, class_name, args, jvm_args=jvm_args)
        except CalledProcessError:
            msg = 'Failed to execute delta calculations (cli failed)'
            logger.exception(msg)
            job.set_status(JobStatus.ERROR)
            job.set_status_details(msg)
            return False
        logger.info('Dataset Delta Finished')
...
def run_job(conf: Config, job_id: str) -> None:
    """
    check that config is valid, run dataset delta, set job status to complete
    :param conf:
    :param job_id:
    :return:
    """
...
def get_args():
    argument_parser = ArgumentParser(description='Execute dataset delta job (typically as a tekton job)')
