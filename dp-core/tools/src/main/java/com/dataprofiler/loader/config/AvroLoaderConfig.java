...
@Deprecated
public class AvroLoaderConfig extends LoaderConfig {
  ...
  public class AvroFileParamsIterator implements Iterator<AvroFileParams> {
    private final AvroLoaderConfig config;
    private int currIndex;
    private final int stride;

    public AvroFileParamsIterator(AvroLoaderConfig config, int startIndex) {
      this.config = config;
    ...
    @Override
    public AvroFileParams next() {
      AvroFileParams p = new AvroFileParams();
  ...
  public class IHateWeirdBugs implements Iterable<AvroFileParams> {
    private AvroLoaderConfig config;

    public Iterator<AvroFileParams> iterator() {
      return new AvroFileParamsIterator(config, iteratorStartIndex);
    }
  }
}
