...
public class SuperTableCalculations {

  private static final Logger logger = Logger.getLogger(SuperTableCalculations.class);

  private static final String PRIMARY_KEY = "PK";
  private static final String NOT_PRIMARY_KEY = "NA";

  public static void execute(DPSparkContext context, String datasetName, String outputFile)
      throws IOException, BasicAccumuloException {
    long starTime = System.currentTimeMillis();

    logger.warn("Calculating statistics for dataset: " + datasetName);

    List<String> tables = new ArrayList<>();
    for (VersionedMetadataObject m :
        new VersionedMetadataObject()
            .scanTables(context, context.getCurrentMetadataVersion(), datasetName)) {
    ...
    if (tables.size() > 0) {
      ...
      logger.warn("No tables found for dataset '" + datasetName + "'");
      return;
    }
    ...
    for (int tableAidx = 0; tableAidx < tables.size(); tableAidx++) {
      ...
      List<String> colNamesA = columnNames(context, datasetName, tableA);

      // Get the columns for tableA: col_name -> (value, count)
      JavaPairRDD<String, Tuple2<String, Long>> colsARDD =
          columns(io, context, datasetName, tableA, ignoredTerms).cache();
      ...
      for (int tableBidx = tableAidx + 1; tableBidx < tables.size(); tableBidx++) {
        ...
        List<String> colNamesB = columnNames(context, datasetName, tableB);

        // Get the columns for tableB: col_name -> (value, count)
        JavaPairRDD<String, Tuple2<String, Long>> colsBRDD =
            columns(io, context, datasetName, tableB, ignoredTerms);
