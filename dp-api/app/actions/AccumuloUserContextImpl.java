...
public class AccumuloUserContextImpl extends Action<AccumuloUserContext> {
  ...
  public CompletionStage<Result> call(Http.Request req) {
    ...
    try {
      ...
      logger.debug("setting rootContext: " + rootContext + " auths: " + rootContext.getAuthorizations());
      logger.debug("setting userContext: " + userContext + " auths: " + userContext.getAuthorizations());
    } catch (BasicAccumuloException e) {
      Result internalError =
          Results.internalServerError(Json.toJson("Failed to get Accumulo " + "context for user"));
      return CompletableFuture.completedFuture(internalError);
    }

    logger.debug("setting new contexts");
    return delegate.call(
        req.addAttr(RulesOfUseHelper.ROOT_CONTEXT_TYPED_KEY, rootContext)
  ...
  public com.dataprofiler.util.Context getContextForUser(Http.Request req)
    ...
    RulesOfUseHelper.UserAttributes ua =
        (UserAttributes) req.attrs().get(RulesOfUseHelper.ROU_ATTRIBUTES_TYPED_KEY);
    logger.debug("creating new context for user: " + ua.getUsername() + " with auths: " + ua.currentAsAuthorizations());
    return new com.dataprofiler.util.Context(context, ua.currentAsAuthorizations());
  }

  public com.dataprofiler.util.Context getContext() throws BasicAccumuloException {
    if (context == null) {
